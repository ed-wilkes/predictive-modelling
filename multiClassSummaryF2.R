...
multiClassSummaryF2 <- function (data, lev = NULL, model = NULL) 
{
  ...
  CM <-  caret::confusionMatrix(data[, "pred"], data[, "obs"], 
                        mode = "everything")
  CM$byClass[,7] <- 5*((CM$byClass[,5] * CM$byClass[,6])/(4*CM$byClass[,5] + CM$byClass[,6])) # convert F1 to F2 score
  ...
  stat_list <- c("Accuracy", "Kappa", "Mean_F2", 
